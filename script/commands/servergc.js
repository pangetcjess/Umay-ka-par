...
module.exports.run = async ({args, api, event, Users}) => {
	try {
		...
		if (!args[0]) {
			...
			let msg = {body: `[!] Your added to the ${groupName}.\nPlease Check your spam or message request.`}
			let msg1 = {body: `Hello ${name}, Welcome to our ${groupName}.`}
		...
		if (args[0] == id1) {
			...
			let msg = {body: `»» User: ${name} added ${name1} in ${groupName}.`}
			let msg1 ={body: `[!] Alert ${name1}\n»» User: ${name} added you in ${groupName}.`}
			let msg2 = {body: `❯ Hello ${name1}, »»Welcome to ${groupName}. `}
			api.sendMessage(msg, event.threadID, event.messageID);
